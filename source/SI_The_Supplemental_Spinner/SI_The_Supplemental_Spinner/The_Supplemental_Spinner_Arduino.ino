...
void winningAnimationTwo() {
	...
	//animate the face to look upward at the golden light
	delay(1200);
	TurnOffLED(3);
	TurnOffLED(13);
	...
	//looks back at the user and winks.
	TurnOffLED(2);
	TurnOffLED(14);
	TurnOffLED(4);
	TurnOffLED(12);
...
void loop() {
	...
	*/
	...
	while (true) {		//this is to wait for user activation for the game to start.

		currentLEDvalue = random(0, 13);					//creating random number between 0 and 13 (13 total variables) for random start
		
		if (analogRead(0) >= 1000) {						//needed to place condition here and not in while loop because of power conflicts between button
				break;
		}
	...
	delay(500);															//delay for button to enter next function as 'OFF' because it can still record as being pressed
	while (analogRead(0) <= 975 || analogRead(0) <= 975.0) {			//reads A0 in the analog pin section
		...
		getPotentiometerSpeed();							//this call is to enable "live" speed updates
	}
